...
class FlagICLModel:
            ...
            for i in range(len(examples_for_task)):
                eg_paris.append(
                    get_detailed_example(
                        examples_for_task[i].get(
                            "instruct", self.query_instruction_for_retrieval
                        ),
    ...
    def encode_queries(
        ...
        if isinstance(queries, str):
            sentences = [
                get_detailed_instruct(self.query_instruction_for_retrieval, queries)
            ]
        else:
            sentences = [
                get_detailed_instruct(self.query_instruction_for_retrieval, q)
                for q in queries
            ]
        ...
        for start_index in tqdm(
            range(0, len(sentences_sorted), batch_size),
            ...
            inputs = self.tokenizer(
                ...
                - len(
                    self.tokenizer("\n<response></s>", add_special_tokens=False)[
                        "input_ids"
                    ]
                ),
            ...
            new_max_length = (
                len(prefix_ids) + len(suffix_ids) + max_length + 8
            ) // 8 * 8 + 8
...
class FlagLLMModel:
    ...
    def encode_queries(
        ...
        """
        if isinstance(queries, str):
            input_texts = get_detailed_instruct(
                self.query_instruction_for_retrieval, queries
            )
        else:
            input_texts = [
                get_detailed_instruct(self.query_instruction_for_retrieval, q)
